                ...
                if(value.size()>0){
                    ...
                    for (String curdate : ts_set) {
                        if(Integer.parseInt(curdate.split("-")[1])==Integer.parseInt(likeDate.split("-")[1])){
                            ...
                            //新增需求 将异常数据在签到表中标红 2020.5.13  hd096846
                            String abnormalSql = "SELECT abnormal_date,morning_sign_in,morning_sign_back,afternoon_sign_in,afternoon_sign_back from uf_AbnormalSign  WHERE abnormal_date like '%"+curdate+"%' AND userid = "+id+"";
                            RecordSet abnormalRecordSet = new RecordSet();
                            abnormalRecordSet.executeSql(abnormalSql);
                            //b.writeLog("当前日期是否有异常申请:>>>"+abnormalSql);
                            int abnormalIndex1=0;//上午签到异常索引
                            int abnormalIndex2=0;//上午签退异常索引
                            int abnormalIndex3=0;//下午签到异常索引
                            int abnormalIndex4=0;//下午签退异常索引
                            String abnormalDate=new String();//异常日期
                            while(abnormalRecordSet.next()){
                                //对异常申请的日期 异常签到 签退进行处理
                                abnormalDate=abnormalRecordSet.getString("abnormal_date");//异常日期
                                String morningSignIn=abnormalRecordSet.getString("morning_sign_in");//上午是否签到
                                String morningSignBack=abnormalRecordSet.getString("morning_sign_back");//上午是否签退
                                String afternoonSignIn=abnormalRecordSet.getString("afternoon_sign_in");//下午是否签到
                                String afternoonSignBack=abnormalRecordSet.getString("afternoon_sign_back");//下午是否签退
                            ...
                            if(abnormalDate.equals(curdate)){
                                //如果在异常流程中上午签到被勾选 标红08：30
                                if(abnormalIndex1==1){
                            %>
                                <font color="red">08:30</font>
                            <%
                            }
                            //如果在异常流程中上午签退被勾选 标红11：45
                                if(abnormalIndex2==2){
                            %>
                                <font color="red">11:45</font>
                            <%
                                }
                            //如果在异常流程中下午签到被勾选 标红13:00
                                if(abnormalIndex3==3){
                            %>
                                <font color="red">13:00</font>
                            <%
                                }
                            //如果在异常流程中下午签退被勾选 标红17:15
                                if(abnormalIndex4==4){
                            %>
                                <font color="red">17:15</font>
                            <%
                                }
                            }else{
                                //遍历当前日期最终打卡时间集合 当天没有外出申请数据
                                for (String time : finalTimeList) {
                                    out.println(time.substring(0,5));
                                } 
                            }
                            out.println("</td>");
                        }
