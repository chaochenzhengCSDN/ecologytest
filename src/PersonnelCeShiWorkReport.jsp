...
<div id="div1">
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr id="trr2">
            ...
            String lastMonth1 = getLastMonth(curDate1);
            //比较当前月份的上一个月以及查询月份
            boolean compareResult= month.compareTo(lastMonth1) == 0&&Integer.parseInt(curDay)>12&&month.compareTo("2020-09")>=0;
			...
			int cnt4 = getId(selectSql3);//节假日天数
            ...
            List<String> curdateList1 = new ArrayList<String>();//用于接收当前日期存在打卡记录的集合
                ...
                String startDateSql="select startdate from hrmresource where id='"+user.getUID()+"'";
                ...
                String endDateSql="select enddate from hrmresource where id='"+user.getUID()+"'";
                ...
                b.writeLog("当前用户合同开始时间："+contractStartDate+";当前用户合同结束时间:"+contractEndDate);
            ...
            "hrmdepartment hde ON hre.departmentid=hde.id WHERE hre.accounttype !=1 and hre.id= "+user.getUID()+" AND CASE WHEN hre.startdate is NULL " +
            ...
            if(areaResult!=null && areaResult.size()>0){
                ...
                double countHours2=0.0;//周六上午旷工小时数
                double countHours3=0.0;//周六下午旷工小时数
                ...
                for (String key : keySet) {
                    ...
                    for (String key3 : value.keySet()) {
                        ...
                        //b.writeLog("当前日期"+key3+"出差+异常处理后的最终集合:"+newFinalTimeList);
                    ...
                    if (value.size() > 0) {
                                ...
                                * */
                                if(afternoonList1Size<=1){
                                    ...
                                    if(firstSign.endsWith("1")&&secondSign.endsWith("2")){
                                        //下午两次打卡数据 则计算迟到早退 13:30:00:1 17:00:00:2
                                        lateTime += Math.max((firstSignForMin - 795), 0);
                                        earlyTime += Math.max((975 - secondSignForMin), 0);
                                        List<String> LeaveList=getCurLeaveCondition(key3,id );
                                        Double relaxedHours =0.0;
                                        for(String leaveRecord:LeaveList){
                                            String startTime=leaveRecord.split(",")[1];
                                            //如果请假开始时间大于17：15，则取17：15,如果开始时间小于13:00,则取13:00，其余情况则取默认值
                                            startTime=getTimeMin(startTime)>975?"16:15":getTimeMin(startTime)<780?"13:00":startTime;
                                            String endTime=leaveRecord.split(",")[2];
                                            //如果请假开始时间大于17：15，则取17：15,否则取默认值
                                            endTime=getTimeMin(endTime)>975?"16:15":getTimeMin(endTime)<780?"13:00":endTime;
                                            relaxedHours +=Double.parseDouble(getDifferenceHours(startTime,endTime));
                                        }
                                        actualWorkingHours +=3.25*2-relaxedHours;
                                    }else if(firstSign.endsWith("1")&&!secondSign.endsWith("2")){
                                        //下午两次打卡记录， 14:00:00:1 16:00:00:4
                                        Double relaxedHours =0.0;
                                        lateTime +=((firstSignForMin-795)>0)?(firstSignForMin-510):0;
                                        countHours3 +=((975-secondSignForMin)>=0)?Double.parseDouble(df11.format((float)(975-secondSignForMin)/60)):0;
                                        List<String> leaveRecordList=getCurLeaveCondition(key3,id );//获取当天打卡记录 计算出请假小时数
                                        for (String leaveRecord : leaveRecordList) {
                                            String startTime=leaveRecord.split(",")[1];//请假开始时间
                                            //如果请假开始时间大于17：15，则取17：15,如果开始时间小于13:00,则取13:00，其余情况则取默认值
                                        ...
                                        //下午两次请假数据 则计算旷工小数  14:00:00:3 16:00:00:4
                                        countHours3 +=(((firstSignForMin-780)>0)?Double.parseDouble(df11.format((float)(firstSignForMin-780)/60)):0)+
                                        ...
                                        for (String leaveRecord : leaveRecordList) {
                                            ...
                                            if(getTimeMin(endTime)>780){
                                                //b.writeLog("当前结束时间2:"+endTime);
                                                //b.writeLog(Double.parseDouble(getDifferenceHours(endTime,"17:15")));
                                                actualWorkingHours +=Double.parseDouble(getDifferenceHours(endTime,"16:15"))*2;//计算请假结束时间到17:15这段时间的实际工时
                                            }
                                        }
                                    }
                                }else if(afternoonList1Size==3){
                                        ...
                                        if(firstSign.endsWith("1")){
                                            //上午三条记录 13：30：00：1  15：00：00：3 17：00：00：4
                                            //上午第一次打卡时间前后计算旷工 第三次打卡时间计算旷工小时数
                                            firstSignForMin = (firstSignForMin-780)>0?firstSignForMin:780;
                                            countHours3 +=((firstSignForMin-780)>0)?Double.parseDouble(df11.format((float)(firstSignForMin-780)/60)):0;
                                            countHours3 +=((secondSignForMin-firstSignForMin)>=0)?Double.parseDouble(df11.format((float)(secondSignForMin-firstSignForMin)/60)):0;
                                            countHours3 +=((975-thirdSignForMin)>=0)?Double.parseDouble(df11.format((float)(975-thirdSignForMin)/60)):0;
                                        }else if(thirdSign.endsWith("1")){
                                            //上午三条记录 13：30：00：3  15：00：00：4 17：00：00：1
                                            //上午第一次打卡时间计算旷工小时数 第三次打卡时间前后计算旷工
                                            countHours3 +=((firstSignForMin-780)>0)?Double.parseDouble(df11.format((float)(firstSignForMin-780)/60)):0;
                                            countHours3 +=((thirdSignForMin-secondSignForMin)>=0)?Double.parseDouble(df11.format((float)(thirdSignForMin-secondSignForMin)/60)):0;
                                            countHours3 +=((975-thirdSignForMin)>=0)?Double.parseDouble(df11.format((float)(975-thirdSignForMin)/60)):0;
                                        }else{
                                            //下午请假两次情况 单独处理计算旷工小时数20201016
                                            double leaveHour1 =((secondSignForMin-firstSignForMin)>=0)?Double.parseDouble(df11.format((float)(secondSignForMin-firstSignForMin)/60)):0;
                                            double leaveHour2 =((forthSignForMin-thirdSignForMin)>=0)?Double.parseDouble(df11.format((float)(forthSignForMin-thirdSignForMin)/60)):0;
                                            countHours3 +=3.25- leaveHour1- leaveHour2;
                                        }
                                }else if(afternoonList1Size>=4){
                                    //下午第一次打卡
                                    String firstSign=afternoonList1.get(0);
                                    ...
                                    if(firstSign.endsWith("1")){
                                        ...
                                        lateTime += Math.max((firstSignForMin - 795), 0);
                                        earlyTime += Math.max((thirdSignForMin - secondSignForMin), 0);
                                        countHours3 +=((975-forthSignForMin)>=0)?Double.parseDouble(df11.format((float)(975-forthSignForMin)/60)):0;
                                        actualWorkingHours +=Double.parseDouble(getDifferenceHours("13:00",thirdSign))*2;//计算下午1点到请假开始时间这段时间的实际工时
                                    }else if(thirdSign.endsWith("1")){
                                        //下午四条记录 13：30：00：3 15:30:00:4 16：00：00：1 17：00：00：2
                                        countHours3 +=((firstSignForMin-780)>0)?Double.parseDouble(df11.format((float)(firstSignForMin-780)/60)):0;
                                        lateTime += Math.max((thirdSignForMin - secondSignForMin), 0);
                                        earlyTime += Math.max((975 - forthSignForMin), 0);
                                        actualWorkingHours +=Double.parseDouble(getDifferenceHours(forthSign,"16:15"))*2;//计算请假结束时间到17：15这段时间的实际工时
                                    }
                                }

                                b.writeLog("周六日期:"+key3+";周六迟到分钟数:"+lateTime+";周六早退分钟数:"+earlyTime+";周六旷工:"+countHours3+";周六上午实际工时："+actualWorkingHours);
                        ...
                        out.println("<td align='center'>" + (countHours+countHours1+countHours2*2+countHours3*2+curdateList.size()*7.5+saturdayList.size()*6.5) + "</td>");//旷工小时数
                         }else{
                            //当月没有打卡记录
                        ...
                        //异常流程次数
                        String abnormalTimeSql = "select nvl(sum(morning_sign_in),0)+nvl(sum(afternoon_sign_in),0)+nvl(sum(morning_sign_back),0)+nvl(sum(afternoon_sign_back),0) from formtable_main_1125 where abnormal_date like '%" + month + "%' and userid=(select id from hrmresource where workcode like '%" + workCode + "%') ";
                        ...
                        for (Integer num1 : getLeaveStandard()) {
                            //获取当月的请假数据
                            String leaveSql = "select case when sum(hours) is null then 0.00 else sum(hours) end as 总天数 from uf_AskForLeave where userid=(select id from hrmresource where workcode='" + workCode + "') and " +
                        ...
                        String OvertimebyHourSql = "SELECT case when SUM (OVERTIME_HOURS) is null then 0.00 else SUM (OVERTIME_HOURS) end FROM uf_WorkOvertime " +
                                "WHERE WORK_DATE LIKE '%" + month + "%' AND userid='"+id+"' AND " +
                        ...
                        //调休工时
                        String paidLeaveTimeSql  =  "SELECT nvl(SUM(overtime_hours),0.00) FROM uf_WorkOvertime WHERE break_off = 0 AND WORK_DATE LIKE '%" + month + "%' AND userid='"+id+"'";
                        ...
                        //剩余调休时间
                        String spareLeaveSql = "select nvl(SUM (OVERTIME_HOURS),0.00) from uf_TimePoolB where userid=(select id from hrmresource where workcode like '%" + workCode + "%') and iseffective=0 ";
                        ...
                        String annualInfoSql = "select total_days,used_days,DECODE(total_days-used_days,null,0,total_days-used_days) spareHours from uf_annual_info where userid=(select id from hrmresource where workcode like '%" + workCode + "%') and year='"+month.split("-")[0]+"' ";
...
//                        //b.writeLog("oldSignInList集合为："+oldSignInList);
            ...
            <%!
                public static Double getLeaveTime1(int num1, String month, String workCode) {
                    ...
                    while (recordSet3.next()) {
                        String eDate = null;
                        //修改结束当月日期的赋值20201221
                        if(Integer.parseInt(month.split("-")[1])==4||
                                Integer.parseInt(month.split("-")[1])==6||
                                Integer.parseInt(month.split("-")[1])==9||
                                Integer.parseInt(month.split("-")[1])==11){
                            eDate = month + "-30";
                        }else if(Integer.parseInt(month.split("-")[1])==1||
                                Integer.parseInt(month.split("-")[1])==3||
                                Integer.parseInt(month.split("-")[1])==5||
                                Integer.parseInt(month.split("-")[1])==7||
                                Integer.parseInt(month.split("-")[1])==8||
                                Integer.parseInt(month.split("-")[1])==10||
                                Integer.parseInt(month.split("-")[1])==12){
                            eDate = month + "-31";
                        }else{
                            if(Integer.parseInt(month.split("-")[0])%4==0){
                                eDate = month + "-29";
                            }else{
                                eDate = month + "-28";
                            }
            ...
            <%!
                /**
                 * 获取各种假期类型的集合 0年休假 1婚假 2产假 8调休 9事假
                 * @return List<Integer>
                 */
