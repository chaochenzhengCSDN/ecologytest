		...
		<table style="width:95%;margin:2%;" border="0" cellpadding="0" cellspacing="0">			
			...
			<%
			   for (Map<String, String> detail : myDetailList) {	
                    ...
                    String days = util.getFiled(detail,"days");
                    //请假小时
                    Double hour = 0.00;
                    DecimalFormat ndf = new DecimalFormat("#.00");
                    //层级判断
                   if(Double.valueOf(days)>0&&getDifferenceMins(start_time,end_time)>0){
                    if("单休周六上午".equals(leaveMark)){
                      if("当日".equals(hourType)){
                          if (getDifferenceMins("8:30",start_time)>=0 && getDifferenceMins(end_time,"11:45")>=0) {
                              hour = Double.valueOf(getDifferenceHours(start_time,end_time));
                          }
                          if (getDifferenceMins("8:30",start_time)<0 && getDifferenceMins(end_time,"11:45")>=0) {
                              hour = Double.valueOf(getDifferenceHours("8:30",end_time));
                          }
                          if (getDifferenceMins("8:30",start_time)>=0 && getDifferenceMins(end_time,"11:45")<0) {
                              hour = Double.valueOf(getDifferenceHours(start_time,"11:45"));
                          }
                          if (getDifferenceMins("8:30",start_time)<0 && getDifferenceMins(end_time,"11:45")<0) {
                              hour = 3.25;
                          }
                      }else if ("开始日".equals(hourType)){
                          if (getDifferenceMins("8:30",start_time)>=0) {
                              hour = Double.valueOf(getDifferenceHours(start_time,"11:45"));
                          }else{
                              hour = 3.25;
                          }
					  }else if ("结束日".equals(hourType)){
                          if (getDifferenceMins(end_time,"11:45")>=0) {
                              hour = Double.valueOf(getDifferenceHours("8:30",end_time));
                          }else{
                              hour = 3.25;
                          }
                     ...
                     if(hour > 0 && hour <= 3.25){
					   ...
					   out.println("<td align='center'>"+ndf.format(hour)+"</td>");
